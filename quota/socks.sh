#!/bin/bash
  while true; do
  sleep 30
  data=($(cat /etc/default/syncron/socks.json | grep '^#÷' | cut -d ' ' -f 2 | sort | uniq))
  if [[ ! -e /etc/limit/socks ]]; then
  mkdir -p /etc/limit/socks
  fi
  for user in ${data[@]}
  do
  xray api stats --server=127.0.0.1:10000 -name "user>>>${user}>>>traffic>>>downlink" >& /tmp/${user}
  getThis=$(cat /tmp/${user} | awk '{print $1}');
  if [[ ${getThis} != "failed" ]]; then
        downlink=$(xray api stats --server=127.0.0.1:10000 -name "user>>>${user}>>>traffic>>>downlink" | grep -w "value" | awk '{print $2}' | cut -d '"' -f2);
        if [ -e /etc/limit/socks/${user} ]; then
        plus2=$(cat /etc/limit/socks/${user});
        if [[ ${#plus2} -gt 0 ]]; then
        plus3=$(( ${downlink} + ${plus2} ));
        echo "${plus3}" > /etc/limit/socks/"${user}"
        xray api stats --server=127.0.0.1:10000 -name "user>>>${user}>>>traffic>>>downlink" -reset > /dev/null 2>&1
        else
        echo "${downlink}" > /etc/limit/socks/"${user}"
        xray api stats --server=127.0.0.1:10000 -name "user>>>${user}>>>traffic>>>downlink" -reset > /dev/null 2>&1
        fi
        else
        echo "${downlink}" > /etc/limit/socks/"${user}"
        xray api stats --server=127.0.0.1:10000 -name "user>>>${user}>>>traffic>>>downlink" -reset > /dev/null 2>&1
        fi
        else
      echo ""
   fi
done
# Check ur Account
for user in ${data[@]}
  do
    if [ -e /etc/socks/${user} ]; then
      checkLimit=$(cat /etc/socks/${user});
      if [[ ${#checkLimit} -gt 1 ]]; then
      if [ -e /etc/limit/socks/${user} ]; then
      Usage=$(cat /etc/limit/socks/${user});
      if [[ ${Usage} -gt ${checkLimit} ]]; then
      exp=$(grep -w "^#÷ $user" "/etc/default/syncron/socks.json" | cut -d ' ' -f 3 | sort | uniq)
      sed -i "/\b$user\b/d" /etc/socks/.socks.db
      sed -i "/^#÷ $user $exp/,/^},{/d" /etc/default/syncron/socks.json
      rm -rf /etc/socks/$user
      rm -rf /etc/limit/socks/$user
      rm -rf /tmp/$user
      systemctl restart xray >> /dev/null 2>&1
      else
      echo ""
      fi
      else
      echo ""
      fi
      else
      echo ""
      fi
      else
      echo ""
    fi
  done
done
