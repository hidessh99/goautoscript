#!/bin/bash
  ...
  if [[ ! -e /etc/limit/shadowsocks2022 ]]; then
  mkdir -p /etc/limit/shadowsocks2022
  fi
  for user in ${data[@]}
  do
  ...
  if [[ ${getThis} != "failed" ]]; then
        ...
        if [ -e /etc/limit/shadowsocks2022/${user} ]; then
        plus2=$(cat /etc/limit/shadowsocks2022/${user});
        if [[ ${#plus2} -gt 0 ]]; then
        plus3=$(( ${downlink} + ${plus2} ));
        echo "${plus3}" > /etc/limit/shadowsocks2022/"${user}"
        xray api stats --server=127.0.0.1:10000 -name "user>>>${user}>>>traffic>>>downlink" -reset > /dev/null 2>&1
        else
        echo "${downlink}" > /etc/limit/shadowsocks2022/"${user}"
        xray api stats --server=127.0.0.1:10000 -name "user>>>${user}>>>traffic>>>downlink" -reset > /dev/null 2>&1
        fi
        else
        echo "${downlink}" > /etc/limit/shadowsocks2022/"${user}"
...
for user in ${data[@]}
  do
    if [ -e /etc/shadowsocks2022/${user} ]; then
      checkLimit=$(cat /etc/shadowsocks2022/${user});
      if [[ ${#checkLimit} -gt 1 ]]; then
      if [ -e /etc/limit/shadowsocks2022/${user} ]; then
      Usage=$(cat /etc/limit/shadowsocks2022/${user});
