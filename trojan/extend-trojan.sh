...
checkPermission
NUMBER_OF_CLIENTS=$(grep -c -E "^#& " "/etc/default/syncron/trojan.json")
...
echo -e "${BB}————————————————————————————————————————————————————${NC}"
grep -E "^#& " "/etc/default/syncron/trojan.json" | cut -d ' ' -f 2-3 | column -t | sort | uniq
echo ""
...
exp=$(grep -wE "^#& $user" "/etc/default/syncron/trojan.json" | cut -d ' ' -f 3 | sort | uniq)
now=$(date +%Y-%m-%d)
d1=$(date -d "$exp" +%s)
d2=$(date -d "$now" +%s)
exp2=$(( (d1 - d2) / 86400 ))
...
sed -i "/#& $user/c\#& $user $exp4" /etc/default/syncron/trojan.json
