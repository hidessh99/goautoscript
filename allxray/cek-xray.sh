...
user_client=("vmess.json" "vless.json" "trojan.json" "socks.json" "shadowsocks.json" "shadowsocks2022.json")
data=($(grep '^#&@' /etc/default/syncron/*.json | cut -d ' ' -f 2 | sort | uniq))
echo -e "${BB}————————————————————————————————————————————————————${NC}"
...
byte=$(cat /etc/limit/allxray/$akun)
