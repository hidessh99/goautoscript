...
# Define the list of JSON files to check
CLIENT_EXISTS=("vmess.json" "vless.json" "trojan.json" "socks.json" "shadowsocks.json" "shadowsocks2022.json")

# Check if the username already exists in any of the JSON files
until [[ $user =~ ^[a-zA-Z0-9_]+$ && $user_exists == '0' ]]; do
    ...
    if ! [[ $user =~ ^[a-zA-Z0-9_]+$ ]]; then
        echo -e "${YB}Invalid username. It must consist of letters, numbers, and underscores.${NC}"
        continue
    fi

    user_exists=0

    # Check if the username exists in any of the JSON files
    for CLIENT_JSON in "${CLIENT_EXISTS[@]}"; do
        if grep -q -w "$user" "/etc/default/syncron/$CLIENT_JSON"; then
            user_exists=1
            break
        fi
    done
    ...
    if [[ $user_exists == '1' ]]; then
        ...
        echo -e "${BB}————————————————————————————————————————————————————${NC}"
        echo -e ""
        echo -e "${YB}A client with the specified name already exists. Please choose another name.${NC}"
        echo -e ""
        echo -e "${BB}————————————————————————————————————————————————————${NC}"
...
# Reset user_exists for the password check
user_exists=0

# Check if the password already exists in any of the JSON files
until [[ $pass =~ ^[a-zA-Z0-9_]+$ && $user_exists == '0' ]]; do
    read -rp "Password (Pass for Socks5): " -e pass

    # Check if the password exists in any of the JSON files
    for CLIENT_JSON in "${CLIENT_EXISTS[@]}"; do
        if grep -q -w "$pass" "/etc/default/syncron/$CLIENT_JSON"; then
            user_exists=1
            break
        fi
    done
    ...
    if [[ $user_exists == '1' ]]; then
        ...
        echo -e "${BB}————————————————————————————————————————————————————${NC}"
        echo -e ""
        echo -e "${YB}A client with the specified password already exists. Please choose another password.${NC}"
        echo -e ""
        echo -e "${BB}————————————————————————————————————————————————————${NC}"
        read -n 1 -s -r -p "Press any key to go back to the menu"
        all-xray
...
},{"id": "'""$uuid""'","email": "'""$user""'"' /etc/default/syncron/vless.json
...
d=$((${c} * 1024 * 1024 * 1024))
if [[ ${c} != "0" ]]; then
  echo "${d}" >/etc/allxray/${user}
fi
DATADB=$(cat /etc/allxray/.allxray.db | grep "^###" | grep -w "${user}" | awk '{print $2}')
if [[ "${DATADB}" != '' ]]; then
  sed -i "/\b${user}\b/d" /etc/allxray/.allxray.db
fi
echo "### ${user} ${exp} ${uuid} 2" >>/etc/allxray/.allxray.db
...
echo -e "Link Akun        : http://$domain:81/allxray/allxray-$user.txt" | tee -a /user/log-allxray-$user.txt
echo -e "————————————————————————————————————————————————————" | tee -a /user/log-allxray-$user.txt
