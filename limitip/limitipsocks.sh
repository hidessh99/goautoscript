...
while true; do
  ...
  for akun in "${data[@]}"; do
    ...
    for ip in "${data2[@]}"; do
      ...
      if [[ "$jum" = "$ip" ]]; then
        echo "$jum" >>/tmp/ipsocks.txt
      else
        echo "$ip" >>/tmp/other.txt
      fi
      jum2=$(cat /tmp/ipsocks.txt | sort | uniq)
      sed -i "/$jum2/d" /tmp/other.txt >/dev/null 2>&1
    done
    
    jum=$(cat /tmp/ipsocks.txt)
    if [[ -z "$jum" ]]; then
      echo >/dev/null
    else
      jum2=$(echo "$jum" | wc -l)
      byte=$(cat /etc/limit/socks/$akun)
      ...
      limit=$(cat /etc/socks/.socks.db | grep -w "$akun" | awk '{print $5}')
