...
CLIENT_EXISTS=$(grep -w $user /etc/default/syncron/socks.json | wc -l)
...
CLIENT_EXISTS=$(grep -w $pass /etc/default/syncron/socks.json | wc -l)
if [[ ${CLIENT_EXISTS} == '1' ]]; then
clear
echo -e "${BB}————————————————————————————————————————————————————${NC}"
echo -e "                 ${WB}Add Socks5 Account${NC}                 "
...
echo "### ${user} ${exp} ${uuid} 2" >>/etc/socks/.socks.db
