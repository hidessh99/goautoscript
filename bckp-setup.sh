...
# Squid already installed in main package list
...
sudo DEBIAN_FRONTEND=noninteractive apt-get -y install iptables iptables-persistent netfilter-persistent squid screen curl jq bzip2 gzip coreutils rsyslog iftop htop zip unzip net-tools sed gnupg bc apt-transport-https build-essential dirmngr libxml-parser-perl lsof openssl openvpn easy-rsa fail2ban stunnel4 socat cron bash-completion ntpdate xz-utils dnsutils lsb-release libnss3-dev libnspr4-dev pkg-config libpam0g-dev libcap-ng-dev libcap-ng-utils libselinux1-dev libcurl4-nss-dev flex bison make libnss3-tools libevent-dev xl2tpd pptpd git speedtest-cli zlib1g-dev python3 python3-pip shc >> /dev/null 2>&1
# Duplicate packages removed (already installed above)
# Core build packages already installed above
# Additional packages for specific requirements only
apt install -y libsqlite3-dev >> /dev/null 2>&1
# Core system packages and DNS configuration
apt update -y >> /dev/null 2>&1
...
# UPDATE TWO STEP - only install missing packages
apt-get --reinstall --fix-missing install -y wget screen >> /dev/null 2>&1
